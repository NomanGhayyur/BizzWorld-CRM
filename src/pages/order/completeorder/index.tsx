...
import SendToClient from "../../../../public/icons/sent.svg";
import ViewInvoice from "../../../../public/icons/invoiceview.svg";
import ReactDOMServer from "react-dom/server";
import Invoice from "../orderInvoice/[id]";
...
const CompeletetOrder: NextPage = () => {
  ...
  const [orderstatus, setOrderStatus] = useState<number | string>(5);
  ...
  const onViewInvoice = useCallback(
    async (
      ...
      event.stopPropagation();
      //@ts-ignore
      const orders = await getOrderDetail(orderToken);
  ...
  const onOrderlViewInvoice = useCallback(
    async (
      orderId: IOrder["order_id"],
      event: React.MouseEvent<HTMLButtonElement, MouseEvent>
    ) => {
      event.stopPropagation();
      //@ts-ignore
      const orders = await getOrderDetail(orderId);
      const newWindow = window.open();
      //@ts-ignore
      newWindow.document.write(
        `<!DOCTYPE>${ReactDOMServer.renderToString(
        ...
        render: (v: any) => (
          <>
            <span
              ...
              title="Send to"
              onClick={onSend.bind(this, v.order_token)}
            >
              <Image src={SendToClient} alt="Alt" width={"25px"} />
            </span>
          </>
        ),
      });
  ...
  const modalColumns = useMemo(() => {
        ...
        render: (v: any) => (
          <>
            <span
              ...
              title="Send to"
              onClick={onOrderSend.bind(this, v.order_id)}
            >
              <Image src={SendToClient} alt="Alt" width={"25px"} />
            </span>
            {user &&
    ...
    return temp;
  }, [user, filterText, onOrderSend, onEdit]);
...
export default CompeletetOrder;
