...
const HaultOrder: NextPage = () => {
  ...
  const [orderstatus, setOrderStatus] = useState<number | string>(7);
  ...
  const columns = useMemo(() => {
    ...
    return temp;
  }, [user, filterText, onUnpick, onCancel, router]);
  ...
  const filtered: Array<IOrderListItem> = useMemo(() => {
    if (filterText && orders?.length) {
      return (
        orders?.filter((v) => {
          return (
            v.order_email?.toLowerCase()?.includes(filterText.toLowerCase()) ||
            `${v.order_id}`.toLowerCase()?.includes(filterText.toLowerCase()) ||
            v.order_website
              ?.toLowerCase()
              ?.includes(filterText.toLowerCase()) ||
            v.order_name?.toLowerCase()?.includes(filterText.toLowerCase())
          );
        }) || []
      );
    }
