...
const RecoveryDeals = React.memo((props) => {
  ...
  const [key, setKey] = useState("recovery");
  ...
  const getRecovery = async (from = pickstart, to = pickend, cpage: number) => {
    const params = {
      params: {
        from,
        to,
        page: cpage,
        orderpaymentstatus_id: 7,
        brand_id: brandID,
      },
    };
  ...
  const {
    data: Recoverypayments,
    isLoading,
    refetch: refetchPickedPayments,
  } = useQuery<Array<IPayment>>(
    [`RecoveryPayments_${pickstart}${brandID}`, pickend, cpage],
    () => getRecovery(pickstart, pickend, cpage),
    {
  ...
  const pickedfiltered: Array<IPayment> = useMemo(() => {
    if (searchPickText) {
      return (
        Recoverypayments?.filter((v: IPayment) => {
          return (
            v.lead_name
              ?.toLowerCase()
    ...
    return Recoverypayments || [];
  }, [searchPickText, Recoverypayments]);
      ...
      >
        ...
        >
          ...
          >
            <Tab
              eventKey="recovery"
              title={`Recovery Deals (${pickedfiltered.length})`}
            >
              <div className="row" style={{ marginBottom: "0.5rem" }}>
                ...
                <div className="col-lg-3">
                  <Input
                    ...
                    placeholder="Search Recoverd Deals"
