...
const PaymentsBuckets = React.memo((props) => {
  ...
  const [key, setKey] = useState("cancel");
  ...
  const {
    ...
    [`CancelledPayments_${pickstart}${brandID}`, pickend, cpage],
    () => getCancelled(pickstart, pickend, cpage),
    {
  ...
  const pickedfiltered: Array<IPayment> = useMemo(() => {
    if (searchPickText) {
      return (
        cancelledpayments?.filter((v: IPayment) => {
          return (
            v.lead_name
              ?.toLowerCase()
  ...
  }, [searchPickText, cancelledpayments]);
      ...
      <Card style={{ marginTop: "0.5rem" }}>
        ...
        >
          ...
          >
            <Tab
              eventKey="cancel"
              title={`Cancel Deals (${pickedfiltered.length})`}
            >
